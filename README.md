...
A single self-contained [AutoHotkey v2](https://www.autohotkey.com/) script.
...
- **Selecting a row** shows that session's transcript in the side pane,
  scrolled to the latest message, with `USER` / `ASSISTANT` headers colored
  and markdown stripped for plain reading.
- **Drag the gray splitter** between the list and the preview to resize.
- **Export Selected** writes the full transcript as a markdown file to
  `%USERPROFILE%\Documents\claude-sessions\` and reveals it in Explorer.
...
单文件、零依赖的 [AutoHotkey v2](https://www.autohotkey.com/) 脚本。
...
- **选中一行**,右侧面板就展示该 session 的对话全文,自动滚到最新一条,
  `USER` / `ASSISTANT` 标题带色高亮,markdown 语法剥掉以纯文本阅读。
- **拖动中间的灰色分隔条**调整列表和预览的左右宽度。
- **Export Selected** 把整段对话导出成 markdown,写到
  `%USERPROFILE%\Documents\claude-sessions\`,完成后 Explorer 弹出并选中该文件。
